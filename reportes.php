...
<section class="reportes-section">
    ...
    <div class="tabs-header">
        <a href="?tab=inasistencias" class="tab-btn<?php if($tab==='inasistencias') echo ' active'; ?>">Inasistencias</a>
        <a href="?tab=llamados" class="tab-btn<?php if($tab==='llamados') echo ' active'; ?>">Llamados de Atención</a>
    </div>
    <div class="tab-content <?php if($tab==='inasistencias') echo 'active'; ?>" id="tab-inasistencias">
        ...
        <div class="table-container">
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><?php echo htmlspecialchars($i['fecha']); ?></td>
                        <td><?php echo htmlspecialchars($i['apellido'] . ', ' . $i['nombre']); ?></td>
                        <td><?php echo $i['anio'] . '° ' . $i['division'] . ' - ' . $i['especialidad']; ?></td>
                        <td><?php echo htmlspecialchars($i['tipo']); ?></td>
                        <td><?php echo $i['justificada'] ? 'Sí' : 'No'; ?></td>
                        <td><?php echo htmlspecialchars($i['motivo']); ?></td>
                    </tr>
                    <?php endforeach; ?>
        ...
        <div class="export-actions">
            <a href="export_inasistencias.php?type=csv&curso=<?php echo urlencode($curso_filter); ?>&estudiante=<?php echo urlencode($estudiante_filter); ?>&fecha_desde=<?php echo urlencode($fecha_desde); ?>&fecha_hasta=<?php echo urlencode($fecha_hasta); ?>" class="btn btn-success"><i class="fas fa-file-excel"></i> Exportar Excel</a>
            <a href="export_inasistencias.php?type=pdf&curso=<?php echo urlencode($curso_filter); ?>&estudiante=<?php echo urlencode($estudiante_filter); ?>&fecha_desde=<?php echo urlencode($fecha_desde); ?>&fecha_hasta=<?php echo urlencode($fecha_hasta); ?>" class="btn btn-danger"><i class="fas fa-file-pdf"></i> Exportar PDF</a>
        </div>
    </div>
    <div class="tab-content <?php if($tab==='llamados') echo 'active'; ?>" id="tab-llamados">
        <form method="GET" class="filters-form">
            <input type="hidden" name="tab" value="llamados">
            <div class="form-row">
                <div class="form-group">
                    <label for="curso">Curso:</label>
                    <select name="curso" id="curso">
